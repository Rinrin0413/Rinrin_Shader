...
float3 posRFA = normalize(PSInput.wP); //reflect(PSInput.wP, normalize(cross(ddx(-PSInput.wP), ddy(PSInput.wP))));
#if !defined(BLEND)
if (TEXTURE_0.Sample(TextureSampler0, PSInput.uv0).a < 0.9) {
	...
	if(PSInput.color.a >= 0.05) {// happ ahajog ayy
		// ↓ clouds Ref
		diffuse.rgb += lerp(float3(0.29, 0.38 ,0.74)*0.3, float3(1.0, 1.0, 0.95), smoothstep(0.4, 0.77, fbm((posRFA.xz+posRFA.xy/2) * 5.7)))*0.3;
	}
}
#endif
// ▲ Reflection from alpha
...
float3 N = saturate(normalize(cross(ddx(-PSInput.pos), ddy(PSInput.pos)))); // Normal map
float3 posR = PSInput.pos;
float3 posN = reflect(PSInput.wP.xyz, N); //reflect(PSInput.wP, normalize(cross(ddx(-PSInput.wP), ddy(PSInput.wP))));
float TRWT = TOTAL_REAL_WORLD_TIME;
float isTop = max(N.y, -N.y);

float polygonWave = clamp(noise(TRWT * 1.7 + posR.xz) - noise(TRWT * 0.39 + posR.xz + float2(TRWT * 1.2 + posR.x + posR.z, TRWT * 1.6 + posR.x + posR.z)), 0.0, 100.0);
float2 posWR = posN.xz*pow(posN.y, -1.2); //noise(posN.xz*pow(posN.y, -1.2));
float wtrReflectTop = fbm(posWR.xy + polygonWave*2.2 *posWR.xy/length(posWR.xy)); //clamp(noise(TRWT * 1.7 + posN.xz) - noise(TRWT * 0.39 + posN.xz + float2(TRWT * 1.2 + posN.x + posN.z, TRWT * 1.6 + posN.x + posN.z)), 0.0, 100.0);
...
float noiseLevel = lerp(0.0, smoothstep(0.1, 1.0, wtrReflect), notReflect); // Noise considering shader

if(PSInput.waterFlag > 0.5) {
   // ↓ Base water color  and  alpha
	// 水面の反射率をそのまま透明度に
	diffuse.a -= lerp(0.2, -0.1, smoothstep(0.1, 1.0, wtrReflectTop)) + 0.06; //;0.08;
	// 色合い
	...
	// 水の線
	float3 waterLine = float3(0.25, 0.25, 0.10);
	diffuse.a += waterLine *smoothstep(0.60391, 0.93726, TEXTURE_0.Sample(TextureSampler0, PSInput.uv0).r);
   	// 水の線 from Light
	diffuse.a += waterLine*0.72 * max(PSInput.uv1.x-.5, 0.) * smoothstep(0.60391, 0.93726, TEXTURE_0.Sample(TextureSampler0, PSInput.uv0).r);

	// ↓ Main reflection
	float4 nightRef = lerp(0.0, float4(1.0, 1.0, 0.8, 1.0), noiseLevel)*0.2;
	float4 dayRef = lerp(0.0, float4(1.2, 1.2, 1.0, lerp(3.0, 1.0, isTop)), noiseLevel)*0.13;
...
const float shadowDepth = 0.54; //0.5
...
#endif
// 草
diffuse.rgb *= lerp(0.65/*0.5*/, 1.0, smoothstep(0.42, 0.6, PSInput.color.g));
// ▲ Side shadow
